...
public class RoomInstance : MonoBehaviour {
	...
	float tileSize = 1;
	Vector3 roomSizeInTiles = new Vector3(60, 0, 60);
	...
	void MakeDoors(){
        ...
        Vector3 spawnPos = transform.position + Vector3.forward * (roomSizeInTiles.z / 1 * tileSize); //- Vector3.forward*(tileSize/4);
		PlaceDoor(spawnPos, doorTop, doorU);
        //bottom door
        spawnPos = transform.position + Vector3.back * (roomSizeInTiles.z / 1 * tileSize); //- Vector3.back*(tileSize/4);
    ...
    //            Vector3 offset = new Vector3((-roomSizeInTiles.x + 1) * tileSize, (roomSizeInTiles.y / 4) * tileSize - (tileSize / 4), 0);
                ...
                ret = new Vector3(tileSize * (float)x, 0, -tileSize * (float)y)  + transform.position; //+offset
                return ret;
            }
        }
